...
int main() {
    ...
    for(int i = 0; i < nums; i++) {
        cout << "我喜歡的第" << (i+1) << "種水果:" << fruits[i] << endl;
    }
    return 0;
}
```
...
int main() {
    ...
    string names[] = {"stu1", "stu2", "stu3", "stu4", "stu5", 
...
int main() {
    ...
    string name[] = {"林小1", "林小2", "林小3", "林小4", "林小5", 
...
### 練習題：二分搜尋練習 (binary_s.cpp)
**題目：** 建立整數陣列 `int num[] = {67, 12, 9, 52, 91, 3}`，讓使用者輸入一個整數，以二分搜尋方法檢查該整數是否存在於陣列中，並顯示查詢結果。

## 字元陣列與字串

### 字串宣告方式
在 C++ 中，使用字串可以使用 string 型別和 char 字元型別：

```cpp
string str = "DevC++";                    // string 型別
...
**注意：** string 型別可以重新賦值，但字元陣列宣告後不能重新賦值。
...
int main() {
    ...
    for(int i = 0; i < namesCount; i++) {
        ...
        cout << names[i] << "得票率為" << fixed << setprecision(2) 
             << scores[i] / totalScores * 100 << "%" << endl << endl;
    }
...
int main() {
    ...
    cout << "本期大樂透電腦選號號碼如下:" << endl << endl;
    
    for(int i = 0; i < num-1; i++) {
        cout << choose[i] << " ";
    }

    cout << endl << endl << "特別號: " << choose[num-1] << endl;
    return 0;
}
```
